/* Stand-alone implementation of algorithm for testing linear equivalence of two canonical 3-to-1 functions */
...
int main(int argc, char *argv[]){
   ...
   if(argc != 3) {
		printf("Usage: Algorithm requires 2 arguments (truth table files representing canonical 3-to-1 functions).\n");
		return 1;
	}

   F = load_vbf_tt_from_file(argv[1]);
   G = load_vbf_tt_from_file(argv[2]);

   if(F.vbf_tt_dimension != G.vbf_tt_dimension){
      fprintf (stderr, "Usage: Functions must have the same dimension.\n");
      vbf_tt_destroy(F);
      vbf_tt_destroy(G);
      return 1;
   }
   ...
   if( ! is_canonical_triplicate (F, &Ft) || ! is_canonical_triplicate (G, &Gt)){
      ...
      printf("Usage: Both functions need to be canonical 3-to-1 functions.\n");
      return 1;
   }
